    ...
    - **Formato:**
        ...
        tools: {
            ...
            getMarsPhotos: tool({
                ...
                parameters: z.object({
                    rover: z.string().describe('The name of the rover. Valid values are curiosity or perseverance. THIS IS REQUIRED.'),
                ...
                // @ts-ignore: AI SDK Zod mismatch prevents execute overload
                execute: async (args: { rover: string, sol?: number, earth_date?: string, camera?: any, page?: number }) => {
                    const { sol, earth_date, camera, page } = args;
                    const rover = args.rover?.toLowerCase().trim() as 'curiosity' | 'perseverance';
                    if (rover !== 'curiosity' && rover !== 'perseverance') {
                        return { error: "You MUST specify the 'rover' parameter as either 'perseverance' or 'curiosity'." };
                    }
                    try {
